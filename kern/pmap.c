...
//first setup UVPT ,than setup UPAGS.'Recursively' the word is said to the local//not the all,so we just setup a table of UVPT not all tables of os.
...
//my commit end
	...
	//my code start here
	//the 'pages' is really in the 0-4MB low mem,and it is belongs pages
	//but not the free pages so I think we can use page_insert(),or
	//boot_map_region();now we use the page_insert();
        //page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
	
	//kern_pgdir[PDX(UPAGES)] = page2pa(pages)|PTE_W|PTE_A;
	//cprintf("pages' size is:%x\n",sizeof(struct PageInfo)*npages);
	//we alloc a page table then insert it to the pgdir with permission
	//when I read what PTE_W means,I find page table/dic entry,maybe
...
//here is the new version,npages*4 because one page address occupy 4B?
	...
	//my code start
	//the first seg is 32K size.//real kern stack. alloced in the entry.S
...
//		cprintf("we are at the while page)free_list loop\n");
//		cprintf("page_free_list:%x\n",page_free_list);	
	}
//cprintf("### we check the page_free_list is:%x\n",page_free_list-pages);
...
page_free(struct PageInfo *pp)
{
	...
	if(pp->pp_ref != 0 || pp->pp_link != NULL){
		...
		/*
			this is the new version here below:
		*/
...
//			cprintf("$$in the page_free:page_free_list->pp_ref :%x$$\n\n",page_free_list->pp_ref);	
			...
			if(pp != page_free_list){
				...
				//so we have already freed it,so do not free it again.
				//so we just do nothing.
//				cprintf("pp :%x = page_free_list:%x\n",pp,page_free_list);
				//panic("the pp == page_free_list,which can form round list.");
			}
		return;
...
/*
	Q:page director means what?
	Q:I return the real address before,then I realize that I should return
	  the virtual address(from the the check_code and website).
	Q:we should know what the page table entry means entry in the 
	  page table? according to following:
...
pgdir_walk(pde_t *pgdir,const void *va, int create)
{
	...
	// Fill this function in
	//my code start
	//I think the pgdir not the entry of the pgdir but the one dir
	//.the word 'relevant' is an 
	//important information(about how to use the pgdir).
	//the line 5,6 means that the creat flag decide whether to alloc
...
{
//cprintf("now we get into boot_map_region\n\n\n");
	//Fill this function in
	//my code start
	//because va is page-aligned,so va must in one page table page
	//so we use pgdir_walk(),
	//because the entries is not clear,so we set two types of entries 
	//with the same perm.
...
//		if(page_free_list-pages <= 0x7fff&& page_free_list-pages>=0x1000){
...
//		}
		...
		*returned_page_table = ((temp_pa))|perm|PTE_P;
		if(temp_va == va+ROUNDUP(size,PGSIZE)-PGSIZE){
			break;
			//if the end is 0xffffffff we have no number get over of it.
		}
//cprintf("e");
...
*/	
	...
	//my code start
	pde_t *p;
	if( (p = pgdir[PDX(va)]) != NULL){
		//we should change pa to va ,so that 
		//we can use p[];beacuse we are in
		//page mode.
		p = (pte_t*)KADDR(PTE_ADDR(p));
		if( p[PTX(va)] != NULL){
		//is it necessary?
			if(pte_store != 0){
				*pte_store = p;
			}
			return pa2page(p[PTX(va)]);	
		}else{
			return NULL;
		}
	}else{
...
page_remove(pde_t *pgdir, void *va)
{
	// Fill this function in
	//my code start here
	//we can infer that if the page exists from the question,we free page 
	//and delet the entry in the page table.
	//why tlb_invalidate I think because when we are at another
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	/*	
	pde_t * check_pgdir = env->env_pgdir;
	uint32_t check_pgtab = (uint32_t)ROUNDOWN(va,PGSIZE);
	if(va+len<(void*)ULIM){
	//should check the pgdir first.one env own 4MB space
		if( ((uint32_t)check_pgdir&PTE_P) && ((uint32_t)check_pgdir&(uint32_t)perm)){
		//pgdir is ok ->4MB is ok
			for(check_pgtab;check_pgtab<=check_pgtab+len;check_pgtab+=PGSIZE){
				void *returned_check_pgtab = pgdir_walk(env->env_pgdir,check_pgtab,0);
				if(((uint32_t)returned_check_pgtab&(uint32_t)PTE_P) && ((uint32_t)returned_check_pgtab&(uint32_t)perm)){
				//page_table is ok ->4KB is ok	
					
				}else{
					//cprintf("[%08x] user_mem_check assertion failure for va %x.\n",env->env_id,check_pgtab);
					user_mem_check_addr = (uintptr_t)check_pgtab; 
					return -E_FAULT;
				}
			}
	...
	cprintf("Function user_mem_check():the mem is OK.\n");
	return 0;
...
check_page_free_list(bool only_low_memory)
{
	...
	if (only_low_memory) {
		...
		*tp[0] = pp2;
		page_free_list = pp1;
		//these code are really skillful.
//		cprintf("pp1:%x   pp2:%x\n",pp1,pp2);
//		cprintf("pp1 next :%x\n",pp1->pp_link);
	}
...
/*
	//exists in lab1 2 3,delete in lab4.
	for (i = 0; i < KSTKSIZE; i += PGSIZE){
...
//	cprintf("before the page_insert.\n");
//	cprintf("page_insert_return_value:%d\n",page_insert(kern_pgdir, pp1, 0x0, PTE_W));
...
//cprintf("$$ at twice before the page_insert pp2 at PGSIZE.\n\n");
	assert(page_insert(kern_pgdir, pp2, (void*) PGSIZE, PTE_W) == 0);
//cprintf("##### after page_alloc and page_insert page_insert:pp2->pp_ref:%x\n",pp2->pp_ref);
	//we should know that the page_insert should also handle the remap Q.
//	assert(check_va2pa(kern_pgdir, PGSIZE) == page2pa(pp2));
//	assert(pp2->pp_ref == 1);
//cprintf("#1157\n");
//cprintf("#####   the page_free_list:%x\n",page_free_list);
...
//cprintf("ptep:%x , PTX(va):%x,va:%x,ptep1:%x\n",ptep,PTX(va),va,ptep1);
	...
	page_free(pp0);
//so the Q is when a page is freed we should set all the values 
//to zero?to make sure it will not cause fault when we use it 
//again. 
